...
Before any commit: if any remote host is in `whoami.apply_when_remote_host`, run `setup_local`; otherwise leave the laptop git author alone. See plugin rule `git-author`.
